...
def generate_patch_vertices(patches, supels, samples, processes):
    progress_bar = tqdm.tqdm(total=2 * len(patches) *
                             samples, desc='preparing')
    ...
    for name, patch in patches.items():
        ...
        vertices = geometry.calc_geometry(
            outer, inner, samples, processes, progress_bar)
...
def compile_tiles(target_resolution, texture_filename, tile_grid_size):
    ...
    resized = compiled.resize(
        (target_resolution, target_resolution), Image.BOX)
...
def main():
    ...
    parser.add_argument('source_resolution', type=positive_int,
                        help='Supersampled source resolution')
    parser.add_argument('target_resolution', type=positive_int,
                        help='Downscaled target resolution')
    parser.add_argument(
        '-o', '--output', help='Output target image filepath', default='texture.png')
    parser.add_argument('-s', '--samples', type=positive_int,
                        help='Number of samples used in geometry', default=256)
    parser.add_argument('-p', '--processes', type=positive_int,
                        help='Number of processes used in preparation stage', default=1)
    args = parser.parse_args()

    if args.source_resolution < args.target_resolution:
        print(
            'Source resolution must not be smaller than target resolution', file=sys.stderr)
        return
